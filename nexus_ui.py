"""NEXUS UI + payment front-end. Install via pip (github pkg) or host as raw url."""
...
def _build_widgets(app):
    ...
    app._widgets.append((app.c.create_window(275, 705, window=app.msg, anchor="nw"), 275, 705, 0, 0))
    ...
    add_button(app, "⏩ SPEED", 1184, 52, 100, lambda: speed_cycle(app), "#273454")
    ...
    add_button(app, "⚡ RANDOM EVENT", 1120, 430, 145, lambda: (app.__dict__.update(cool=0.), app.shock(), render(app)), "#5d3a95")
...
def speed_cycle(app):
    app.spd = {.5:1.,1.:2.,2.:.25,.25:.5}[app.spd]; note(app, f"⏩ SPEED {app.spd}x", "#8ea0ff")
...
def sdraw(app):
    ...
    T(app, 1120,240,"LEVERAGE · CLICK",9,"#77839d",tg="s"); T(app, 1120,285,"MARGIN HEALTH",9,"#8290a8",tg="s")
...
def render(app):
    ...
    T(app, 275,164,f"{app.tf} · closes {max(0,int(SPAN[app.tf]-(time.time()-app.started)))}s · vol {app.vol*100:.2f}% · {app.news}",10,"#7d89a3")
    ...
    for i in range(12):
        ...
        R(app, 950,y-9,min(1078,950+w),y+9, fill="#301827" if i < 6 else "#102d27")
    ...
    eq = app.equity(); mr = (eq/max(app.frozen(),1)*100) if app.POS else 100
    T(app, 1120,120,money(app.cash),15,"#edf1fa",bold=True); T(app, 1120,140,f"EQUITY {money(eq)}",11,"#8ea0ff",bold=True)
    T(app, 1120,157,f"AVAIL {money(app.cash-app.frozen())} · FROZEN {money(app.frozen())}",8,"#7c889f")
    T(app, 1120,195,f"{app.lot} shares",16,"#edf1fa"); T(app, 1120,258,f"{app.lev}×",13,"#edf1fa")
    R(app, 1110,300,1110+165*max(0,min(100,mr))/100,315, fill="#2ee6a0" if mr > 80 else "#f5bc4b" if mr > 55 else "#ff5c7a")
    T(app, 1192,330,f"MRG {mr:.0f}% · FEE {app.FEE:.2%} · BRW {app.BORROW*100:.4f}%",8,"#7c889f","center")
    ...
    T(app, 275,732,f"RANDOM NEWS · {app.news} · FEAR {app.fear} · FEES {money(app.fees)}",10,"#8ea0ff")
...
def _draw_pay(app):
    ...
    T(app, 650,415,f"当前可用 {money(app.cash-app.frozen())} · 首充+10% · ≥5万+5% · 加密+2% · 20%幸运+1~8%",10,"#f5bc4b","center")
    for j,p in enumerate(app.PEND[:4]): T(app, 330,445+j*18,f"⏳ {p[2]} {money(abs(p[1]))} {'入账' if p[1] > 0 else '提现'} · {max(0,p[0]-time.time()):.0f}s",9,"#8ea0ff")
...
def pay(app):
    ...
    q = simpledialog.askfloat("DEPOSIT" if app.MODE == "dep" else "WITHDRAW",
          ...
          minvalue=1, maxvalue=cap if app.MODE == "dep" else av) or 0
    if not q: return
    ...
    if app.MODE == "dep":
        ...
        if dl > 0:
            app.PEND.append([time.time()+dl, net, name]); app.MODE = ""; note(app, f"⏳ {name} {money(net)} 处理中 · 约{dl}s到账")
        else:
            app.cash += net; app.MODE = "bonus"; confetti(app); app.r.after(2300, lambda: (app.__dict__.update(MODE=""), render(app)))
            note(app, f"✓ {name} +{money(net)} · 含 bonus {money(b)}"); app.r.bell()
    else:
        if q+fee > av: messagebox.showwarning("WITHDRAW", "Exceeds available cash (margin is frozen)."); return
        if random.random() < risk:
        ...
        app.PEND.append([time.time()+max(dl2,1), -q, name])
